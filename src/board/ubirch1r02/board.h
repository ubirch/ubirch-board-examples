...
static inline void board_init() {
  ...
  PORT_SetPinMux(BOARD_LED0_PORT, BOARD_LED0_PIN, BOARD_LED0_ALT);
  GPIO_PinInit(BOARD_LED0_GPIO, BOARD_LED0_PIN, &OUTFALSE);

  PORT_SetPinMux(BOARD_BUTTON0_PORT, BOARD_BUTTON0_PIN, BOARD_BUTTON0_ALT);
...
static inline void board_nmi_disable() {
  ...
  PORT_SetPinMux(BOARD_NMI_PORT, BOARD_NMI_PIN, BOARD_NMI_ALT);
